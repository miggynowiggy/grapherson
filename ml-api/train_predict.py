...
def start(filename):
    if os.path.isfile("label_list"):
        ...
        while True:
            ...
            raw_features = extract.start(filename)
            print("\n")
            print(raw_features)
            ...
            emotionalStability = clf1.predict([[baseline_angle, slant_angle]])
            mentalEnergy = clf2.predict([[letter_size, pen_pressure]])
            modesty = clf3.predict([[letter_size, top_margin]])
            harmony = clf4.predict([[line_spacing, word_spacing]])
            discipline = clf5.predict([[slant_angle, top_margin]])
            concentration = clf6.predict([[letter_size, line_spacing]])
            communicativeness = clf7.predict([[letter_size, word_spacing]])
            socialIsolation =  clf8.predict([[line_spacing, word_spacing]])

            print("Emotional Stability: ", emotionalStability)
            print("Mental Energy: ", mentalEnergy)
            print("Modesty: ", modesty)
            print("Harmony: ", harmony)
            print("Lack of Dicipline: ", discipline)
            print("Poor Concentration: ", concentration)
            print("Non Communicativeness: ", communicativeness)
            print("Social Isolation: ", socialIsolation)

            return {
                'emotionalStability': emotionalStability[0],
                'mentalEnergy': mentalEnergy[0],
                'modesty': modesty[0],
                'harmony': harmony[0],
                'discipline': discipline[0],
                'concentration': concentration[0],
                'communicativeness': communicativeness[0],
                'socialIsolation': socialIsolation[0]
            }
